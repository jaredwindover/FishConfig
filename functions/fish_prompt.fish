function fish_prompt --description 'Write out the prompt'
	...
	if test $last_status -ne 0
		set prompt_status ' ' (set_color $fish_color_error) "$crash" \
	...
	echo -n -s (set_color white -b purple) (date "+%_H:%M") $normal (set_color $fish_color_user) "$USER" $normal ' ' (set_color $color_cwd) (prompt_pwd) $normal (__fish_git_prompt) $normal $prompt_status "$mode_str" "> "
end
